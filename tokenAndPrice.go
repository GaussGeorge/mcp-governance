...
// 返回一个切片，包含键值对字符串（下游工具名和对应的令牌数），用于注入 _meta。
func (gov *MCPGovernor) SplitTokens(ctx context.Context, tokenleft int64, methodName string) ([]string, error) {
	downstreamNames := gov.callMap[methodName]
	...
	downstreamPriceSum, _ := gov.RetrieveDSPrice(ctx, methodName)
	...
	logger("[令牌分配]: 下游总价格为 %d, 共有 %d 个下游服务 (%s), 每个下游分配到的额外盈余为 %d\n",
		downstreamPriceSum, size, gov.nodeName, tokenleftPerDownstream)

	for _, downstreamName := range downstreamNames {
		// 读取特定下游节点的价格（键名格式：方法名-节点名）
		downstreamPriceString, _ := gov.priceTableMap.Load(methodName + "-" + downstreamName)
...
func (gov *MCPGovernor) RetrieveDSPrice(ctx context.Context, methodName string) (int64, error) {
	if len(gov.callMap[methodName]) == 0 {
		logger("[获取下游价格]:    %s 没有下游服务\n", methodName)
		return 0, nil
	}
	// 从价格表中加载该方法的下游价格（这个 key 存储的是聚合后的价格）
	downstreamPrice_string, ok := gov.priceTableMap.Load(methodName)
...
func (gov *MCPGovernor) RetrieveTotalPrice(ctx context.Context, methodName string) (string, error) {
	...
	downstreamPrice, _ := gov.RetrieveDSPrice(ctx, methodName)
...
// SetOwnPrice 直接设置自身价格（用于测试或手动干预）
func (gov *MCPGovernor) SetOwnPrice(price int64) {
	gov.priceTableMap.Store("ownprice", price)
}
...
func (gov *MCPGovernor) UpdateOwnPrice(congestion bool) error {
	...
	logger("[更新自身价格]:  拥塞状态: %t, 当前价格: %d, 步长: %d\n", congestion, ownPrice, gov.priceStep)

	if congestion {
		// 如果检测到拥塞，价格直接上涨一个步长 (Step)
		ownPrice += gov.priceStep
...
func (gov *MCPGovernor) UpdatePrice(ctx context.Context) error {
	// 1. 获取当前自身价格
	ownPriceInterface, _ := gov.priceTableMap.Load("ownprice")
	...
	if gov.priceStrategy == "expdecay" {
		if adjustment > 0 {
			// 如果连续多次涨价
			if gov.consecutiveIncreases >= 2 {
				// 且次数超过阈值，按 decayRate 进行衰减
				// 新调整幅度 = 原幅度 * (衰减率 ^ 连续次数)
				// 也就是涨得越来越慢
				adjustment = int64(float64(adjustment) * math.Pow(gov.decayRate, float64(gov.consecutiveIncreases)))
				logger("[价格步长衰减]: 价格步长减少比例 %f ** %d\n", gov.decayRate, gov.consecutiveIncreases)
			}
			gov.consecutiveIncreases++ // 增加连续上涨计数器
	...
	reservePrice := int64(math.Max(float64(gov.guidePrice), 0))
...
func (gov *MCPGovernor) UpdateDownstreamPrice(ctx context.Context, method string, nodeName string, downstreamPrice int64) (int64, error) {

	// --- 策略 A: 取最大值 (Maximal) 或 平均值 (Mean) ---
	if gov.priceAggregation == "maximal" || gov.priceAggregation == "mean" {
		...
		downstreamPrice_old, loaded := gov.priceTableMap.Load(method)
		if !loaded {
		...
		if downstreamPrice > downstreamPrice_old.(int64) {
			gov.priceTableMap.Store(method, downstreamPrice)
		...
		gov.priceTableMap.Range(func(key, value interface{}) bool {
		...
		// 仅更新当前请求涉及的方法的价格
		gov.priceTableMap.Store(method, maxPrice)
		...
		// 3. 将差价传播给所有依赖该节点的方法
		// 遍历调用映射表
		for methodName, downstreamNames := range gov.callMap {
			for _, downstreamName := range downstreamNames {
				if downstreamName == nodeName {
					// 该方法依赖此节点，更新其总价格
					// 加载当前价格 -> 加上差价 -> 存回
					methodPrice, _ := gov.priceTableMap.Load(methodName)
					methodPrice = methodPrice.(int64) + diff
					gov.priceTableMap.Store(methodName, methodPrice)
					// 记录日志
