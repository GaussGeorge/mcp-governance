...
// AddObservedDelay 累加观测到的业务延迟 (用于 pinpointLatency 模式)
// 外部调用者（如代理服务器）在每次请求完成后调用此方法，报告实际的请求处理延迟。
// latencyCheck() 后台协程会定期读取累积延迟并与阈值比较，据此调整价格。
// 使用 atomic 操作保证高并发安全。
func (gov *MCPGovernor) AddObservedDelay(d time.Duration) {
	atomic.AddInt64((*int64)(&gov.observedDelay), int64(d))
	gov.Increment()
}
...
func (gov *MCPGovernor) latencyCheck() {
	...
	for range time.Tick(gov.priceUpdateRate) {
		// 原子读取并重置累积延迟
		observedMs := time.Duration(atomic.SwapInt64((*int64)(&gov.observedDelay), 0)).Milliseconds()
		count := gov.GetCount()

		// 安全检查：如果本周期内没有请求，则认为无拥塞（跳过）
		// 这是必要的，因为 count=0 时 latencyThreshold*0=0，
		// 任何残余的 observedMs>0 都会误判为拥塞
		if count == 0 {
			gov.UpdateOwnPrice(false)
			continue
		}

		// 更新自身价格：
		// 判定逻辑：如果 观察到的总延迟 > (延迟阈值 * 请求数量)
		// 这实际上是在比较：平均延迟(observedDelay / count) 是否超过了 单次请求的延迟阈值(latencyThreshold)
		gov.UpdateOwnPrice(observedMs > gov.latencyThreshold.Milliseconds()*count)
	}
}
