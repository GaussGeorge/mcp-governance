...
func main() {
	// 命令行参数
	mode := flag.String("mode", "quick", "测试模式: quick(快速验证) / full(完整测试) / single(单策略) / ablation(消融对照)")
	...
	latencyThreshold := flag.Duration("latency-threshold", 500*time.Microsecond, "Rajomon 延迟阈值")
	...
	case "ablation":
		...
		fmt.Printf("目标: %s\n", *ablationTarget)
		fmt.Println("目的: 对比不同参数配置对策略效果的影响")
		fmt.Println()
		...
		case "rajomon":
			fmt.Println(">>> 运行 Rajomon 参数消融对照组（priceStep × latencyThreshold）")
			_, err := runner.RunRajomonAblation(quick)
			if err != nil {
				fmt.Printf("消融实验失败: %v\n", err)
				os.Exit(1)
			}
		case "static":
			fmt.Println(">>> 运行静态限流参数消融对照组（QPS 阈值）")
			_, err := runner.RunStaticRateLimitAblation(quick)
			if err != nil {
				fmt.Printf("消融实验失败: %v\n", err)
				os.Exit(1)
			}
		case "capacity":
			fmt.Println(">>> 运行后端容量消融对照组（maxConcurrency）")
			_, err := runner.RunCapacityAblation(quick)
			if err != nil {
				fmt.Printf("消融实验失败: %v\n", err)
				os.Exit(1)
			}
		case "all":
			fmt.Println(">>> 运行完整消融实验（Rajomon + 静态限流 + 后端容量）")
