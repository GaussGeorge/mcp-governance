...
type TestConfig struct {
	...
	// --- HTTP 客户端配置 ---
	HTTPClientTimeout time.Duration // HTTP 客户端请求超时（与 integration 保持一致）
}
...
func DefaultTestConfig() *TestConfig {
	return &TestConfig{
		...
		BudgetWeights: []float64{0.3, 0.4, 0.3}, // 预算权重：与 integration 保持一致，中等预算占比最高
		...
		// HTTP 客户端
		HTTPClientTimeout: 30 * time.Second, // 与 integration 保持一致
	}
}
...
// 而 nil 字段表示"保持 DefaultTestConfig 的默认值"，实现"单因素对照"。
