...
// ==================== 共享后端过载模拟器 ====================
// 为保证三种策略公平对比，所有策略的后端使用相同的过载模型。
// 治理层(限流/定价)在前端决定是否放行请求，放行后的请求进入相同的后端。
// 如果治理有效，后端不会过载；如果治理失效，后端同样会退化。
...
type backendSimulator struct {
	...
	mockDelayVar       time.Duration
}

func newBackendSimulator(cfg *TestConfig) *backendSimulator {
	return &backendSimulator{
		maxConcurrency: int64(cfg.MaxServerConcurrency),
		overloadScale:  cfg.OverloadLatencyScale,
		mockDelay:      cfg.MockDelay,
		mockDelayVar:   cfg.MockDelayVar,
	}
}

// process 模拟后端处理，包含并发追踪和过载延迟
// 返回 error 表示后端过载拒绝（并发超过最大容量）
func (bs *backendSimulator) process() error {
	current := atomic.AddInt64(&bs.concurrentRequests, 1)
	defer atomic.AddInt64(&bs.concurrentRequests, -1)

	if current > bs.maxConcurrency {
		return fmt.Errorf("服务器过载：当前并发 %d 超过容量 %d", current, bs.maxConcurrency)
	}

	loadRatio := float64(current) / float64(bs.maxConcurrency)
	extraDelay := time.Duration(0)
	if loadRatio > 0.7 {
		overloadFactor := math.Pow((loadRatio-0.7)/0.3, 2) * bs.overloadScale
		extraDelay = time.Duration(float64(bs.mockDelay) * overloadFactor)
	}

	simulateProcessing(bs.mockDelay+extraDelay, bs.mockDelayVar)
	return nil
}
...
func StartNoGovernanceServer(cfg *TestConfig) (*ServerInstance, error) {
	...
	// 使用共享后端模拟器
	backend := newBackendSimulator(cfg)
	...
	server.RegisterTool(nogovernance.MCPTool{
		...
		if err := backend.process(); err != nil {
			return nil, err
		}
...
func StartStaticRateLimitServer(cfg *TestConfig) (*ServerInstance, error) {
	...
	// 使用共享后端模拟器（与无治理、Rajomon 相同的后端过载模型）
	backend := newBackendSimulator(cfg)
